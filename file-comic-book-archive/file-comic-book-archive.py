...
def extract_cbz_cbt(file_path, output_dir):
    os.makedirs(output_dir, exist_ok=True)

    def extract_recursive(archive, path):
        for entry in archive.infolist():
            if entry.is_dir():
                folder_path = os.path.join(path, entry.filename)
                os.makedirs(folder_path, exist_ok=True)
                extract_recursive(archive, folder_path)
            else:
                with archive.open(entry) as file:
                    with Image.open(file) as image:
                        image.save(os.path.join(path, entry.filename))

    if file_path.endswith('.cbz'):
        with zipfile.ZipFile(file_path, 'r') as archive:
            extract_recursive(archive, output_dir)
    elif file_path.endswith('.cbt'):
        with tarfile.open(file_path, 'r') as archive:
            extract_recursive(archive, output_dir)

if __name__ == '__main__':
    file_path = 'path/to/your/cbt_or_cbz_file'
    output_dir = 'path/to/output/directory'
    extract_cbz_cbt(file_path, output_dir)
...
def save_cbz(procedure, run_mode, image, n_drawables, drawables, file, metadata, config, data):
    ...
    def add_layers_to_archive(image, archive, current_path=''):
        layers = image.list_layers()
        for layer in layers:
            if layer.is_group():
                group_path = os.path.join(current_path, layer.get_name())
                archive.write('', arcname=group_path)
                add_layers_to_archive(layer, archive, current_path=group_path)
            else:
                store_layer(image, layer, current_path)
...
def save_cbt(procedure, run_mode, image, n_drawables, drawables, file, metadata, config, data):
    #note differences
    #got rid of def write_file_str because we arnt dealing with zip files anymore
    # GUI Code
    Gimp.progress_init("Exporting Comic Book Archive (cbt) image")

    if run_mode == Gimp.RunMode.INTERACTIVE:
        GimpUi.init('python-fu-file-cbt-save')
    ...
    def store_layer(image, drawable, path):
        ...
        pdb_proc.run(pdb_config)
        if (os.path.exists(tmp)):
            cbaffFile.add(tmp, path) #changed this to add since we are callimng back on cbaffFile
            os.remove(tmp)
        else:
            print("Error removing ", tmp)
